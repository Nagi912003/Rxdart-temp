import 'package:flutter/material.dart';
import 'package:rxdarttemp/models/animal.dart';
import 'package:rxdarttemp/models/person.dart';

import '../bloc/search_result.dart';

class SearchResultView extends StatelessWidget {
  final Stream<SearchResult?> searchResultStream;
  const SearchResultView({Key? key, required this.searchResultStream})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<SearchResult?>(
      stream: searchResultStream,
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          final result = snapshot.data;
          if (result is SearchResultHasError) {
            return const Text('got error');
          } else if (result is SearchResultLoading) {
            return const CircularProgressIndicator();
          } else if (result is SearchResultNoResults) {
            return const Text('no results');
          } else if (result is SearchResultWithResults) {
            final results = result.results;
            return Expanded(
                child: ListView.builder(
              itemCount: results.length,
                  itemBuilder: (context, index) {
                    final item = results[index];
                    final String title;
                    if(item is Animal){
                      title = 'Animal';
                    }else if(item is Person){
                      title = 'Person';
                    }else{
                      title = 'Unknown';
                    }
                    return ListTile(
                      title: Text(title),
                      subtitle: Text(item.toString()),
                    );
                  }
            ));
          }else{
            return const Text('unknown');
          }
        } else {
          return const Text('waiting');
        }
      },
    );
  }
}
